...
// --- Deep-link helpers ---
function __getPoIdFromUrl(){
  try {
    const u = new URL(window.location.href);
    const q = u.searchParams.get('po');
    if (q) return q;
  } catch(_) {}
  if (window.location.hash && window.location.hash.startsWith('#po=')) {
    return window.location.hash.slice(4);
  }
  try { return sessionStorage.getItem('open_po_on_ready') || null; } catch(_) { return null; }
}
function __persistPoIdToSession(poId){
  try { if (poId) sessionStorage.setItem('open_po_on_ready', String(poId)); } catch(_) {}
}
...
async function checkUserGroupMembership() {
  ...
  try {
    ...
    const data = await res.json();
      try { const url = new URL(window.location.href); url.searchParams.set('po', String(poId)); history.pushState({ po: String(poId) }, '', url.toString()); __persistPoIdToSession(poId); if (window.DEBUG_DEEPLINK) console.log('[DL] pushState', url.toString()); } catch(_){}
...
function wireOpenPoHandler() {
      ...
      openModal(`
        <div class="flex items-center justify-between mb-3 pr-32">
          <div class="text-lg font-semibold">PO Preview – ${escapeHtml(found.poId)}</div>
            <button id="copy-po-link" class="inline-flex items-center justify-center w-8 h-8 rounded hover:bg-slate-100" title="Copy link to this PO"><svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 24 24\" class=\"w-4 h-4\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\"><path d=\"M10 13a5 5 0 0 0 7.07 0l2.83-2.83a5 5 0 1 0-7.07-7.07L11 4\" /><path d=\"M14 11a5 5 0 0 0-7.07 0L4.1 13.83a5 5 0 0 0 7.07 7.07L13 20\" /></svg></button>
          <div class="flex items-center gap-2">
  ...
  // Drain any early deep-link queue
  try {
    if (Array.isArray(window.__poOpenQueue) && window.__poOpenQueue.length) {
      const pre = window.__poOpenQueue.shift();
      if (window.DEBUG_DEEPLINK) console.log('[DL] draining queue -> open', pre);
      setTimeout(() => showPreviewModal(pre), 0);
    }
  } catch (e) { console.warn('[DL] drain failed', e); }
...
async function onReady(){
  await revealAppUI();
  try {
    const qPo = __getPoIdFromUrl();
    if (qPo) {
      window.__poDeepLinkHandled = true;
      if (window.DEBUG_DEEPLINK) console.log('[DL] onReady open', qPo);
      setTimeout(() => showPreviewModal(qPo), 0);
    }
  } catch(e) { console.warn('[DL] onReady failed', e); }
...
window.addEventListener('popstate', () => {
  const id = __getPoIdFromUrl();
  if (id) { window.__poDeepLinkHandled = true; if (window.DEBUG_DEEPLINK) console.log('[DL] popstate open', id); showPreviewModal(id); }
  else { try { if (window.DEBUG_DEEPLINK) console.log('[DL] popstate close'); closeModal(); } catch(_){} }
});
window.addEventListener('hashchange', () => {
  const id = __getPoIdFromUrl();
  if (id) { window.__poDeepLinkHandled = true; if (window.DEBUG_DEEPLINK) console.log('[DL] hashchange open', id); showPreviewModal(id); }
  else { try { if (window.DEBUG_DEEPLINK) console.log('[DL] hashchange close'); closeModal(); } catch(_){} }
});
...
// Final deep-link guard after window load
window.addEventListener('load', () => {
  if (window.__poDeepLinkHandled) return;
  const id = __getPoIdFromUrl();
  if (id) { window.__poDeepLinkHandled = true; if (window.DEBUG_DEEPLINK) console.log('[DL] load open', id); showPreviewModal(id); }
});

// Global delegated handler for the copy link button
document.addEventListener('click', function(e){
  const btn = e.target.closest('#copy-po-link');
  if (!btn) return;
  const id = (history.state && history.state.po) || __getPoIdFromUrl();
  if (!id) { try { toast("No PO selected", true); } catch(_){} if (window.DEBUG_DEEPLINK) console.warn('[DL] copy no id'); return; }
  const link = window.location.origin + window.location.pathname + "?po=" + encodeURIComponent(String(id));
  if (window.DEBUG_DEEPLINK) console.log('[DL] copy', link);
  try {
    ...
    try { toast("Link copied"); } catch(_) {}
  } catch (e) {
    try { toast("Could not copy link", true); } catch(_) {}
  }
});
