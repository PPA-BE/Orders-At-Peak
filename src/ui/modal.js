...
export function openModal(html) {
  const urlCleanup = () => {
    try {
      ...
      if (u.searchParams.has('po')) { u.searchParams.delete('po'); history.replaceState({}, '', u.toString()); }
      else if (window.location.hash && window.location.hash.startsWith('#po=')) { history.replaceState({}, '', window.location.pathname + window.location.search); }
      if (window.DEBUG_DEEPLINK) console.log('[DL] cleaned URL on close');
    } catch (_) {}
  };
